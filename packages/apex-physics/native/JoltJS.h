...
// SPDX-FileCopyrightText: 2026 Jonathan Villaverde
...
/// Compiled Apex tire-force callback.
///
/// Model 0 leaves Jolt's defaults untouched, model 1 evaluates Apex Brush,
/// model 2 evaluates Apex TMeasy and model 3 suppresses the central Jolt
/// contact while the experimental nine-point TypeScript path owns grip.
/// Keeping the callback inside the WASM removes the per-wheel JS round trip
/// for the two production force models.
class ApexTireForceBridge
{
public:
	static constexpr uint	cMaxWheels = 8;
	...
	void					SetWheeledVehicleController(WheeledVehicleController &inController)
	{
		inController.SetTireMaxImpulseCallback([this](uint inWheelIndex, float &outLongitudinalImpulse, float &outLateralImpulse, float inSuspensionImpulse, float inLongitudinalFriction, float inLateralFriction, float inLongitudinalSlip, float inLateralSlip, float inDeltaTime) {
			...
			if (mModel != 1 && mModel != 2)
				return;

			const float safe_delta = std::max(inDeltaTime, 0.001f);
			const float load = std::max(0.0f, inSuspensionImpulse / safe_delta);
			const uint patch_count = std::max(1u, mPatchesPerContact);
			const TireForces forces = mModel == 1
				? EvaluateBrush(load / float(patch_count), inLongitudinalSlip, inLateralSlip, GetSurface(inWheelIndex))
				: EvaluateTMeasy(load / float(patch_count), inLongitudinalSlip, inLateralSlip, GetSurface(inWheelIndex));
			const float patch_scale = float(patch_count);
			const float longitudinal_force = forces.mLongitudinalForce * patch_scale;
			const float lateral_force = forces.mLateralForce * patch_scale * mLateralGripMultiplier;

			outLongitudinalImpulse = std::abs(longitudinal_force) * safe_delta;
			outLateralImpulse = std::abs(lateral_force) * safe_delta;

			if (inWheelIndex < cMaxWheels)
			{
				WheelTelemetry &telemetry = mTelemetry[inWheelIndex];
				telemetry.mLongitudinalCapacity = forces.mLongitudinalCapacity * patch_scale;
				telemetry.mLateralCapacity = forces.mLateralCapacity * patch_scale * mLateralGripMultiplier;
				telemetry.mAligningMoment = forces.mAligningMoment * patch_scale * mLateralGripMultiplier;
				const uint wheel_bit = 1u << inWheelIndex;
				if ((mEvaluatedWheelMask & wheel_bit) == 0)
				{
					mEvaluatedWheelMask |= wheel_bit;
					mEvaluatedContactCount += patch_count;
				}
			}
		});
	}

	void					SetModel(uint inModel)
	{
		mModel = inModel <= 3 ? inModel : 0;
	}

	void					SetPatchesPerContact(uint inPatchCount)
	{
		mPatchesPerContact = std::max(1u, inPatchCount);
	}

	void					SetOperatingScales(float inGripScale, float inStiffnessScale)
	{
		mOperatingGripScale = std::max(0.0f, inGripScale);
		mPressureStiffnessScale = std::max(0.0f, inStiffnessScale);
	}

	void					SetLateralGripMultiplier(float inMultiplier)
	{
		mLateralGripMultiplier = std::max(0.0f, inMultiplier);
	}

	void					SetWheelSurface(uint inWheelIndex, float inLongitudinalMu, float inLateralMu, float inPeakSlipRatio, float inPeakSlipAngleRadians)
	{
		if (inWheelIndex >= cMaxWheels)
			return;
		Surface &surface = mSurfaces[inWheelIndex];
		surface.mLongitudinalMu = std::max(0.0f, inLongitudinalMu);
		surface.mLateralMu = std::max(0.0f, inLateralMu);
		surface.mPeakSlipRatio = std::max(0.0001f, inPeakSlipRatio);
		surface.mPeakSlipAngle = std::max(0.0001f, inPeakSlipAngleRadians);
	}

	void					BeginStep()
	{
		mEvaluatedWheelMask = 0;
		mEvaluatedContactCount = 0;
		for (WheelTelemetry &telemetry : mTelemetry)
				telemetry = WheelTelemetry();
	}

	void					EvaluateSample(uint inWheelIndex, float inVerticalLoad, float inSlipRatio, float inSlipAngle)
	{
		if (inWheelIndex >= cMaxWheels || (mModel != 1 && mModel != 2))
			return;
		const uint patch_count = std::max(1u, mPatchesPerContact);
		const TireForces forces = mModel == 1
			? EvaluateBrush(inVerticalLoad / float(patch_count), inSlipRatio, inSlipAngle, GetSurface(inWheelIndex))
			: EvaluateTMeasy(inVerticalLoad / float(patch_count), inSlipRatio, inSlipAngle, GetSurface(inWheelIndex));
		const float patch_scale = float(patch_count);
		mSampleLongitudinalForce = forces.mLongitudinalForce * patch_scale;
		mSampleLateralForce = forces.mLateralForce * patch_scale * mLateralGripMultiplier;
		WheelTelemetry &telemetry = mTelemetry[inWheelIndex];
		telemetry.mLongitudinalCapacity = forces.mLongitudinalCapacity * patch_scale;
		telemetry.mLateralCapacity = forces.mLateralCapacity * patch_scale * mLateralGripMultiplier;
		telemetry.mAligningMoment = forces.mAligningMoment * patch_scale * mLateralGripMultiplier;
	...
	float					GetSampleLongitudinalForce() const
	{
		return mSampleLongitudinalForce;
	}

	float					GetSampleLateralForce() const
	{
		return mSampleLateralForce;
	}

	uint					GetEvaluatedContactCount() const
	{
		return mEvaluatedContactCount;
	}

	float					GetLongitudinalCapacity(uint inWheelIndex) const
	{
		return inWheelIndex < cMaxWheels ? mTelemetry[inWheelIndex].mLongitudinalCapacity : 0.0f;
	}

	float					GetLateralCapacity(uint inWheelIndex) const
	{
		return inWheelIndex < cMaxWheels ? mTelemetry[inWheelIndex].mLateralCapacity : 0.0f;
	}

	float					GetAligningMoment(uint inWheelIndex) const
	{
		return inWheelIndex < cMaxWheels ? mTelemetry[inWheelIndex].mAligningMoment : 0.0f;
	}

private:
	struct Surface
	{
		float				mLongitudinalMu = 1.32f;
		float				mLateralMu = 1.32f;
		float				mPeakSlipRatio = 0.105f;
		float				mPeakSlipAngle = 0.10995574f;
	};

	struct TireForces
	{
		float				mLongitudinalForce = 0.0f;
		float				mLateralForce = 0.0f;
		float				mAligningMoment = 0.0f;
		float				mLongitudinalCapacity = 0.0f;
		float				mLateralCapacity = 0.0f;
	};
	...
	static float			Clamp(float inValue, float inMinimum, float inMaximum)
	{
		return std::max(inMinimum, std::min(inMaximum, inValue));
	}

	static float			SmoothStep(float inEdge0, float inEdge1, float inValue)
	{
		const float normalized = Clamp((inValue - inEdge0) / (inEdge1 - inEdge0), 0.0f, 1.0f);
		return normalized * normalized * (3.0f - 2.0f * normalized);
	}

	static float			LoadSensitiveCapacity(float inLoad, float inReferenceLoad, float inExponent, float inMu)
	{
		const float load_ratio = std::max(0.02f, inLoad / inReferenceLoad);
		return inReferenceLoad * std::pow(load_ratio, inExponent) * inMu;
	}

	const Surface &			GetSurface(uint inWheelIndex) const
	{
		return mSurfaces[inWheelIndex < cMaxWheels ? inWheelIndex : 0];
	}

	TireForces				EvaluateTMeasy(float inLoad, float inSlipRatio, float inSlipAngle, const Surface &inSurface) const
	{
		TireForces result;
		if (inLoad < 1.0f)
			return result;

		result.mLongitudinalCapacity = LoadSensitiveCapacity(inLoad, 3800.0f, 0.96f, inSurface.mLongitudinalMu * mOperatingGripScale);
		result.mLateralCapacity = LoadSensitiveCapacity(inLoad, 3800.0f, 0.96f, inSurface.mLateralMu * mOperatingGripScale);
		const float normalized_longitudinal = inSlipRatio / inSurface.mPeakSlipRatio;
		const float normalized_lateral = std::tan(inSlipAngle) / std::max(0.0001f, std::tan(inSurface.mPeakSlipAngle));
		const float combined_slip = std::hypot(normalized_longitudinal, normalized_lateral);
		float adhesion;
		if (combined_slip <= 1.0f)
		{
			const float slope = 5.4f * mPressureStiffnessScale;
			adhesion = Clamp(slope * combined_slip / (1.0f + (slope - 1.0f) * combined_slip), 0.0f, 1.0f);
		}
		else
		{
			const float distance = combined_slip - 1.0f;
			adhesion = 0.86f + 0.14f * std::exp(-0.64f * distance * distance);
		}
		const float direction_scale = combined_slip > 0.00001f ? adhesion / combined_slip : 0.0f;
		result.mLongitudinalForce = result.mLongitudinalCapacity * normalized_longitudinal * direction_scale;
		result.mLateralForce = -result.mLateralCapacity * normalized_lateral * direction_scale;
		const float trail = 0.082f * (1.0f - 0.84f * SmoothStep(0.55f, 2.25f, std::abs(normalized_lateral)));
		result.mAligningMoment = -result.mLateralForce * trail;
		return result;
	}

	TireForces				EvaluateBrush(float inLoad, float inSlipRatio, float inSlipAngle, const Surface &inSurface) const
	{
		TireForces result;
		if (inLoad < 1.0f)
			return result;

		result.mLongitudinalCapacity = LoadSensitiveCapacity(inLoad, 3800.0f, 0.94f, inSurface.mLongitudinalMu * mOperatingGripScale);
		result.mLateralCapacity = LoadSensitiveCapacity(inLoad, 3800.0f, 0.94f, inSurface.mLateralMu * mOperatingGripScale);
		const float longitudinal_stiffness = inLoad * 11.5f * mPressureStiffnessScale;
		const float cornering_stiffness = inLoad * 10.2f * mPressureStiffnessScale;
		float longitudinal_force = result.mLongitudinalCapacity * std::tanh(
			longitudinal_stiffness * inSlipRatio / std::max(1.0f, result.mLongitudinalCapacity));
		const float tangent = std::tan(Clamp(inSlipAngle, -1.50796447f, 1.50796447f));
		const float absolute_tangent = std::abs(tangent);
		const float critical_tangent = 3.0f * result.mLateralCapacity / std::max(1.0f, cornering_stiffness);
		float lateral_axis_force;
		if (absolute_tangent >= critical_tangent)
			lateral_axis_force = result.mLateralCapacity * (tangent < 0.0f ? -1.0f : tangent > 0.0f ? 1.0f : 0.0f);
		else
		{
			const float first = cornering_stiffness * tangent;
			const float second = cornering_stiffness * cornering_stiffness
				/ (3.0f * result.mLateralCapacity) * absolute_tangent * tangent;
			const float third = cornering_stiffness * cornering_stiffness * cornering_stiffness
				/ (27.0f * result.mLateralCapacity * result.mLateralCapacity) * tangent * tangent * tangent;
			lateral_axis_force = first - second + third;
		}
		float lateral_force = -lateral_axis_force;
		const float normalized_longitudinal = longitudinal_force / std::max(1.0f, result.mLongitudinalCapacity);
		const float normalized_lateral_force = lateral_force / std::max(1.0f, result.mLateralCapacity);
		const float demand = std::hypot(normalized_longitudinal, normalized_lateral_force);
		const float combined_scale = demand > 1.0f ? 1.0f / demand : 1.0f;
		result.mLongitudinalForce = longitudinal_force * combined_scale;
		result.mLateralForce = lateral_force * combined_scale;
		const float normalized_lateral_slip = std::abs(inSlipAngle) / inSurface.mPeakSlipAngle;
		const float trail = 0.075f * (1.0f - 0.82f * SmoothStep(0.65f, 2.1f, normalized_lateral_slip));
		result.mAligningMoment = -result.mLateralForce * trail;
		return result;
	}

	uint					mModel = 0;
	uint					mPatchesPerContact = 1;
	float					mOperatingGripScale = 1.0f;
	float					mPressureStiffnessScale = 1.0f;
	float					mLateralGripMultiplier = 1.0f;
	uint					mEvaluatedWheelMask = 0;
	uint					mEvaluatedContactCount = 0;
	float					mSampleLongitudinalForce = 0.0f;
	float					mSampleLateralForce = 0.0f;
	std::array<Surface, cMaxWheels> mSurfaces;
	std::array<WheelTelemetry, cMaxWheels> mTelemetry;
};
